...
export default function ChoiceModal<T extends string | number>({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4" onClick={onCancel}>
      <div
        className="w-full max-w-sm rounded-2xl border border-fuchsia-400/40 bg-[#140d1a] p-5 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <h3 className="text-xl font-bold text-fuchsia-100">{title}</h3>
        ...
        <div className="mt-2 flex flex-col gap-2">
          {options.map((o) => (
            <button
              ...
              className="rounded-lg bg-white/10 px-4 py-2 text-left text-sm font-semibold hover:bg-fuchsia-500/30"
            >
              {o.label}
            </button>
          ))}
